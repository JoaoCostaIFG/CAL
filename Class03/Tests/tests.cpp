...
int testNP(string name, vector<Point> &pontos, double dmin, NP_FUNC func,
           string alg) {
...
int testNPRandConstX(int size, string name, double dmin, NP_FUNC func,
                     string alg) {
  vector<Point> pontos;
...
void testNearestPoints(NP_FUNC func, string alg) {
  cout << "algorithm; data set; time elapsed (ms); distance; point1; point2"
       << endl;
  int maxTime = 10000;
 ...
 *   setNumThreads(2);
 *   testNearestPoints(nearestPoints_DC_MT, "Divide and conquer with 2
 * threads");
 * }
 */
